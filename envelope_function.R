...
envelope_LSMLE <- function(y, X, Z, theta, linkmu="logit", linkphi="log", LSMLE=T, main.title = "Envelope", faixa.fixed = NULL, labels.fixed = NULL) { 
  ...
  for(j in 1:B){		
    ...
    if(LSMLE==T){
      fit <- LSMLE_BETA(y=ygen, X=X, Z=Z, qoptimal=T, q0=1, m=3, L=0.02, qmin=0.5, spac =0.02, method="BFGS", startV="CP", linkmu=linkmu, linkphi=linkphi)
    }
    else{
      fit <- LSMLE_BETA(y=ygen, X=X, Z=Z, qoptimal=F, q0=1, m=3, L=0.02, qmin=0.5, spac=0.02, method="BFGS", startV="CP", linkmu=linkmu, linkphi=linkphi)
    }
  ...
  v <- qqnorm(res_rp2, main=main.title, xlab="Normal quantiles", ylab="Residuals", ylim=faixa, pch="+", las =1, cex=1.5, cex.lab=1.5, cex.axis=1.4, cex.main=2.0)
  ...
  par(new=T)
  #
  qqnorm(res_min_rp2,axes=F,main = "",xlab="",ylab="",type="l",ylim=faixa)
  par(new=T)
  qqnorm(res_max_rp2,axes=F,main = "",xlab="",ylab="", type="l",ylim=faixa)
  par(new=T)
  qqnorm(res_mean_rp2,axes=F,xlab="",main = "", ylab="", type="l",ylim=faixa, lty=2)
}#ends function